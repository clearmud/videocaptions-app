...
import { generateCaptionsAPI } from '../services/api/video';
...
export function EditorPage() {
  ...
  const handleGenerateCaptions = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await generateCaptionsAPI(videoFile, duration);
      const generatedCaptions = response.captions;
      ...
      setError(err.message || "Sorry, I couldn't generate captions. Please try again.");
    } finally {
      setIsLoading(false);
    }
  }, [videoFile, duration, refreshUser]);
            ...
            {user && (
              <div className="flex items-center gap-4 border-l border-gray-700 pl-6">
                <div className="text-sm text-gray-300">
                  <span className="font-semibold">{user.minutes_remaining}</span> min remaining
                </div>
